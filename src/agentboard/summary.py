...
context_file_writer_var = contextvars.ContextVar('context_file_writer')
...
class FileWriter(object):
    ...
    def __init__(self, logdir, static="./static", flush_secs=120, **kwargs):
        ...
        context_file_writer_var.set(self)
...
def text(name, data, **kwargs):
    ...
    """
    writer = context_file_writer_var.get(KEY_CONTEXT_FILE_WRITER)
    if writer is None or isinstance(writer, str):
        print("DEBUG: FileWriter is missing in contextvars, Should be declared using 'with' statement, such as 'with FileWriter(logdir=\"./log\") as writer:'")
        return 
    try:
        ...
        log_data[KEY_WORKFLOW_TYPE] = kwargs[KEY_WORKFLOW_TYPE] if KEY_WORKFLOW_TYPE in kwargs else WORKFLOW_TYPE_PROCESS # Optional field
        ...
        # context var.get("key") will return str if not set
...
def dic(name, data, **kwargs):
    """ Functions to log dict type data
        data: list of dict
    """
    ## write logs
    import json
...
def agent_loop(name, data, agent_name, process_id, **kwargs):
    ...
    try:
        if not (isinstance(data, dict) or isinstance(data, str)):
            print ("ERROR: agent_loop input data should be in python dict or str format, input is %s" % str(type(data)) )
            return
        log_data = {}
...
## AI Agent Workflow Class
class BasePipeline(object):

    def __init__(self, **kwargs):
        """
            agent_name: str, e.g. agent 1
            process_id: str, e.g. RAG
        """
        self.log_data = {}
...
def display_emb(embedding, float_num = 4):
    """
        default display format float ".2f"
    """
    embedding_str = []
    if isinstance(float_num, int):
        float_format_template = "{:."+ str(float_num) +"f}"
    else:
        float_format_template = "{:.4f}"
    for d in embedding:
        d_str = float_format_template.format(d)
        embedding_str.append(d_str)
    embedding_display = ",".join(embedding_str)
    return embedding_display

class RAGPipeline(BasePipeline):
    """
        Write a log type to local store with data_type as RAG
        required fields:
        input
        output
    """
    def __init__(self, **kwargs):
        super().__init__()
        # require field
        self.log_data[KEY_DATA] = { KEY_INPUT: {}, KEY_OUTPUT: {} }
        self.log_data[KEY_DATA_TYPE] = DATA_TYPE_RAG
        self.log_data[KEY_TIMESTAMP] = get_current_timestamp_milli()
        self.log_data[KEY_WORKFLOW_ID] = get_workflow_id()
        self.log_data[KEY_WORKFLOW_TYPE] = kwargs[KEY_WORKFLOW_TYPE] if KEY_WORKFLOW_TYPE in kwargs else WORKFLOW_TYPE_PROCESS
        # Optional field
        self.log_data[KEY_NAME] = kwargs[KEY_NAME] if KEY_NAME in kwargs else ""
        self.log_data[KEY_AGENT_NAME] = kwargs[KEY_AGENT_NAME] if KEY_AGENT_NAME in kwargs else ""
        self.log_data[KEY_PROCESS_ID] = kwargs[KEY_PROCESS_ID] if KEY_PROCESS_ID in kwargs else ""  

    def input(self, query, embedding, **kwargs):
        """
            query: list of str
            embedding: 2D list of float
            **kwargs: dict
        """
        input_list = []
        assert isinstance(query, list), "RAG Pipeline Input Query should be type list of str..."
        assert isinstance(embedding, list), "RAG Pipeline Input embedding should be type 2D list of float..."
        assert len(query) == len(embedding), "RAG input query list size %d and embedding list size %d mismatch" % (len(query), len(embedding))
        float_num = kwargs[DATA_FORMAT_KEY_FLOAT_NUM] if DATA_FORMAT_KEY_FLOAT_NUM in kwargs else DATA_FORMAT_FLOAT_NUM_DEFAULT
        for (q, emb) in zip(query, embedding):
            query_dict = {}
            query_dict[KEY_RAG_QUERY] = q   # str
            query_dict[KEY_EMBEDDING] = emb # 1D list of float
            query_dict[KEY_EMBEDDING_DISPLAY] = display_emb(emb, float_num)
            input_list.append(query_dict)
        input_dict = {}
        input_dict[KEY_RAG_QUERY] = input_list
        if KEY_DATA in self.log_data and KEY_INPUT in self.log_data[KEY_DATA]:
            self.log_data[KEY_DATA][KEY_INPUT] = input_dict
    
    def output(self, docs, scores, key_doc_id="doc_id", key_content="content", key_doc_embedding="embedding", key_doc_score="score",**kwargs):
        """
            docs: 2D list of json dict of document, 
                e.g. [ [{"doc_id": "123", "content": "Doc Content 123"}, {"doc_id": "456", "content": "Doc Content 456"}], [] ]
            score: 2D list of float representing relevance score
        """
        output_dict = {}
        assert isinstance(docs, list), "RAG Pipeline Node Output docs should be list of dict..."
        assert isinstance(scores, list), "RAG Pipeline Node Output score should be list of float..."
        assert len(docs) == len(scores), "RAG Pipeline Node Output size mismatch docs size %d, score size %d" % (len(docs), len(score))

        # save output to list of dict, merge score to each retrived doc
        output_list = []
        for (doc_list, score_list) in zip(docs, scores):
            res_doc_list = []
            for (doc, score) in zip(doc_list, score_list):
                doc[KEY_RAG_SCORE] = score 
                res_doc_list.append(doc)
            output_list.append(res_doc_list)
        

        output_dict[KEY_RAG_DOCS] = output_list
        output_dict[KEY_DOC_ID] = key_doc_id
        output_dict[KEY_DOC_CONTENT] = key_content
        output_dict[KEY_DOC_EMBEDDING] = key_doc_embedding
        output_dict[KEY_DOC_SCORE] = key_doc_score
        if KEY_DATA in self.log_data and KEY_OUTPUT in self.log_data[KEY_DATA]:
            self.log_data[KEY_DATA][KEY_OUTPUT] = output_dict
